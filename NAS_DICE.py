...
from DICE import yf_dataset_withdemo, model_2, func_analysis_test_error_D0406, collate_fn
import pickle
import wandb
...
def analysis_architecture(args, inputmodelpath, inputdatatrainpath, inputnhidden, n_clusters):
    ...
    dataloader_train = DataLoader(data_train, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn)
    
    data_test = yf_dataset_withdemo(X_test, y_test, args.n_hidden_fea, mode='test')
    dataloader_test = DataLoader(data_test, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn)
